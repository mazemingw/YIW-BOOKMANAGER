...
public List getList() {
   ...
   String s1 = "select * from books order by id asc"; 
...
public void saveList(List list) {
   ...
   try {
    ...
    while (it.hasNext()) {
     ...
     s1 = "insert into books(bookname,jieyue,bookid) values(?,?,?)";
     ps = conn.prepareStatement(s1);
     ps.setString(1, name);
     ps.setString(2, desc);
...
public void update(List list) {
   ...
   try {
    while (it.hasNext()) {
     ...
     switch (col) {
     case 1:
      str = "update books set bookname=? where id=?";
      break;
     case 2:
      str = "update books set jieyue=? where id=?";
      break;
     case 3:
      str = "update books set bookid=? where id=?";
     }
     ps = conn.prepareStatement(str);
