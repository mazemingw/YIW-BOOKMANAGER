...
public List getList() {
   ...
   String s1 = "select * from employees order by id asc"; //用id排序
...
public void saveList(List list) {
   ...
   try {
    ...
    while (it.hasNext()) {
     ...
     s1 = "insert into employees(name,password,sex) values(?,?,?)";
     ps = conn.prepareStatement(s1);
     ps.setString(1, name);
     ps.setString(2, desc);
...
public void update(List list) {
   ...
   try {
    while (it.hasNext()) {
     ...
     switch (col) {
     case 1:
      str = "update employees set name=? where id=?";
      break;
     case 2:
      str = "update employees set password=? where id=?";
      break;
     case 3:
      str = "update employees set sex=? where id=?";
     }
     ps = conn.prepareStatement(str);
