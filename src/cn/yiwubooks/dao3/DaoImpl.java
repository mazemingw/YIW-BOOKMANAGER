...
public List getList() {
   ...
   String s1 = "select * from admin order by id asc"; //用id排序
...
public void saveList(List list) {
   ...
   try {
    ...
    while (it.hasNext()) {
     ...
     s1 = "insert into admin (name,password,sex) values(?,?,?)";
     ps = conn.prepareStatement(s1);
     ps.setString(1, name);
     ps.setString(2, desc);
...
public void deleteList(List list) {
   String str = "delete from admin where id=?";
   int id = 0;
   Iterator it = list.iterator();
   Connection conn = JDBConnection.getConn();
   PreparedStatement ps = null;
...
public void update(List list) {
   ...
   try {
    while (it.hasNext()) {
     ...
     switch (col) {
     case 1:
      str = "update admin set name=? where id=?";
      break;
     case 2:
      str = "update admin set password=? where id=?";
      break;
     case 3:
      str = "update admin set sex=? where id=?";
     }
     ps = conn.prepareStatement(str);
