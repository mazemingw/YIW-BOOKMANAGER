...
public List getList() {
   ...
   String s1 = "select * from user1 order by id asc"; //用学号排序
...
public void saveList(List list) {
   ...
   try {
    ...
    while (it.hasNext()) {
     ...
     s1 = "insert into user1(mingzi,mima,banji) values(?,?,?)";
     ps = conn.prepareStatement(s1);
     ps.setString(1, name);
     ps.setString(2, desc);
...
public void deleteList(List list) {
   String str = "delete from user1 where id=?";
   int id = 0;
   Iterator it = list.iterator();
   Connection conn = JDBConnection.getConn();
   PreparedStatement ps = null;
...
public void update(List list) {
   ...
   try {
    while (it.hasNext()) {
     ...
     switch (col) {
     case 1:
      str = "update user1 set mingzi=? where id=?";
      break;
     case 2:
      str = "update user1 set mima=? where id=?";
      break;
     case 3:
      str = "update user1 set banji=? where id=?";
     }
     ps = conn.prepareStatement(str);
